...
class loginlog_evac extends baseTable {

	public function getTableName() {
		return "loginlog_evac";
	}
	...
	public function getSQL() {
		...
		$sql .= " INNER JOIN mv_4_guardian s ON ".$this->getTableName().".manager_id = s.guardian_id";
		$sql .= " WHERE ".$this->getTableName().".move_flg = 0";
		$sql .= "   AND ".$this->getTableName().".clients='parent'";
		$sql .= " UNION ALL ";												// UNION ALLで順番になる
		$sql .= " SELECT ".$this->getTableName().".* FROM ".$this->getTableName()." ".$this->getTableName()." ";
		$sql .= " INNER JOIN mv_2_teacher s ON ".$this->getTableName().".manager_id = s.teacher_id";
		$sql .= " WHERE ".$this->getTableName().".move_flg = 0";
		$sql .= "   AND ".$this->getTableName().".clients='teacher'";
	...
	public function getDeleteSQL() {
		...
		$sql .= " INNER JOIN mv_2_teacher m ON m.teacher_id = t.manager_id ";
		$sql .= " WHERE t.move_flg = 0";
		$sql .= "   AND t.clients='teacher'";
		$del_list[] = $sql;

		$sql = "";
		$sql .= " DELETE t FROM ".$this->getTableName()." t ";
		$sql .= " INNER JOIN mv_4_guardian m ON m.guardian_id = t.manager_id ";
		$sql .= " WHERE t.move_flg = 0";
		$sql .= "   AND t.clients='parent'";
		$del_list[] = $sql;
