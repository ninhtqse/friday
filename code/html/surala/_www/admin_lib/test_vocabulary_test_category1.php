...
function select_menu() {
	...
	if (!$_SESSION['sub_session']['test_type_num'] ) {
		$html .= "<br>\n";
		$html .= "テスト種別１を設定するすらら英単語種類を選択してください。<br>\n";
	}
...
function category_list($ERROR) {
	...
	$html .= "<br>\n";
	$html .= "インポートする場合は、テスト種別１csvファイル（S-JIS）を指定しCSVインポートボタンを押してください。<br>\n";
	...
	if (!$_SESSION['authority'] || ($_SESSION['authority'] && array_search(MAIN."__".SUB."__add",$_SESSION['authority'])===FALSE)) {
		...
		$html .= "<input type=\"submit\" value=\"テスト種別１新規登録\">\n";
		$html .= "</form>\n";
	}

	if ($_SESSION['sub_session']['test_type_num']) {
		$where .= " AND ttn.test_type_num='".$_SESSION['sub_session']['test_type_num']."'";
	}

	//upd start hirose 2018/11/26 すらら英単語　ディレクトリ作成
//	$sql  = "SELECT tc1.* FROM " . T_MS_TEST_CATEGORY1 . " tc1" .
//			" LEFT JOIN ".T_MS_TEST_TYPE." ttn ON tc1.test_type_num=ttn.test_type_num".
	$sql  = "SELECT tc1.*,ttn.write_type FROM " . T_MS_TEST_CATEGORY1 . " tc1" . 
			" INNER JOIN ".T_MS_TEST_TYPE." ttn ON tc1.test_type_num=ttn.test_type_num".
	//upd end hirose 2018/11/26 すらら英単語　ディレクトリ作成
			" WHERE tc1.mk_flg='0' ".$where;
	...
	if (!$test_count) {
		$html .= "<br>\n";
		$html .= "今現在登録されているテスト種別１は有りません。<br>\n";
		return $html;
	}
	...
	echo $_SESSION['sub_session']['s_page'];
	...
	if ($result = $cdb->query($sql)) {
		...
		$html .= "<br>\n";
		$html .= "修正する場合は、修正するテスト種別１の詳細ボタンを押してください。<br>\n";
		$html .= "<div style=\"float:left;\">登録テスト種別１総数(".$test_count."):PAGE[".$page."/".$max_page."]</div>\n";
		if ($back > 0) {
		...
		while ($list=$cdb->fetch_assoc($result)) {
			...
			$temp_path = STUDENT_VOCABLARY_TEMP_DIR . $list['write_type'] . "/". $list['test_type_num'] . "/";
			$temp_path2 = STUDENT_VOCABLARY_TEMP_DIR . $list['write_type'] . "/". $list['test_type_num'] . "/". $list['test_category1_num'] . "/";
			
			if (!file_exists($temp_path)) {
					@mkdir($temp_path, 0777);
					@chmod($temp_path, 0777);
				}
			if (!file_exists($temp_path2)) {
					@mkdir($temp_path2, 0777);
					@chmod($temp_path2, 0777);
				}
			$material_temp_path = MATERIAL_TEMP_DIR . "vocabulary/". $list['write_type'] ."/". $list['test_type_num'] . "/";
				if (!file_exists($material_temp_path)) {
					@mkdir($material_temp_path,0777);
					@chmod($material_temp_path,0777);
				}
			$button_path = "../student/images/button/vocabulary/". $list['write_type'] ."/". $list['test_type_num'] . "/";
			...
			$html .= "<td>".$list['test_category1_num']."</td>\n";
			$html .= "<td>".$list['test_category1_name']."</td>\n";
...
function addform($ERROR) {
	...
	$make_html->set_file(TANGO_TEST_CATEGORY1_FORM);
...
//	$配列名['置換コメント名'] = array('コマンド名'=>'値','コマンド名'=>'値',....);
	...
	$INPUTS['CATEGORY1ID'] 		= array('result'=>'plane','value'=>"---");
	$INPUTS['CATEGORY1NAME'] 		= array('type'=>'text','name'=>'test_category1_name','size'=>'50','value'=>$_POST['test_category1_name']);
...
function viewform($ERROR) {
	...
	if (ACTION) {
		...
		$sql = "SELECT * FROM " . T_MS_TEST_CATEGORY1 . " WHERE test_category1_num='".$_POST['test_category1_num']."' AND mk_flg='0' AND test_type_num='".$_POST['test_type_num']."' LIMIT 1;";
	...
	if (!$test_category1_num) { $test_category1_num = "---"; }
	$display_html = "";
	foreach($L_DISPLAY as $key => $val) {
		if ($val == "") { continue; }
		$newform = new form_parts();
	...
	$INPUTS['CATEGORY1ID'] 		= array('result'=>'plane','value'=>$test_category1_num);
	$INPUTS['CATEGORY1NAME'] 		= array('type'=>'text','name'=>'test_category1_name','size'=>'50','value'=>$test_category1_name);
...
function check() {
	...
	if (!$_POST['test_category1_name']) { $ERROR[] = "テスト種別１名が未入力です。"; }
	else {
		if (MODE == "add") {
			$sql  = "SELECT * FROM " . T_MS_TEST_CATEGORY1 . " WHERE mk_flg='0'".
				" AND test_type_num='".$_POST['test_type_num']."'".
				" AND test_category1_name='".$_POST['test_category1_name']."'";

		} else {
			$sql  = "SELECT * FROM " . T_MS_TEST_CATEGORY1 . " WHERE mk_flg='0'".
				" AND test_type_num='".$_POST['test_type_num']."'".
				" AND test_category1_num!='".$_POST['test_category1_num']."' AND test_category1_name='".$_POST['test_category1_name']."'";
		}
		if ($result = $cdb->query($sql)) {
			$count = $cdb->num_rows($result);
		}
		if ($count > 0) { $ERROR[] = "入力されたテスト種別１名は既に登録されております。"; }
	}
...
//	$配列名['置換コメント名'] = array('コマンド名'=>'値','コマンド名'=>'値',....);
	...
	$INPUTS[TESTTYPENAME] 		= array('result'=>'plane','value'=>$L_TYPE_LIST[$test_type_num]);
	$INPUTS[CATEGORY1ID] 		= array('result'=>'plane','value'=>$test_category1_num);
	$INPUTS[CATEGORY1NAME] 		= array('result'=>'plane','value'=>$test_category1_name);
...
function add() {
	...
	$sql = "SELECT MAX(test_category1_num) AS max_id FROM " . T_MS_TEST_CATEGORY1 .";";
	if ($result = $cdb->query($sql)) {
		$list = $cdb->fetch_assoc($result);
	}
	...
	$INSERT_DATA['test_category1_num'] 	= $add_id;
	$INSERT_DATA['test_category1_name'] 	= $_POST['test_category1_name'];
	...
	$ERROR = $cdb->insert(T_MS_TEST_CATEGORY1,$INSERT_DATA);
...
//	$where = " WHERE test_category1_num='".$_POST['test_category1_num']."' LIMIT 1;";
	...
	$ERROR = $cdb->update(T_MS_TEST_CATEGORY1,$INSERT_DATA,$where);
...
function up() {
	...
	$sql  = "SELECT * FROM " . T_MS_TEST_CATEGORY1 . " WHERE test_category1_num='".$_POST['test_category1_num']."' AND test_type_num='".$_POST['test_type_num']."' LIMIT 1;";
	if ($result = $cdb->query($sql)) {
		$list = $cdb->fetch_assoc($result);
		$m_test_category1_num = $list['test_category1_num'];
		$m_list_num = $list['list_num'];
	}
	if (!$m_test_category1_num || !$m_list_num) { $ERROR[] = "移動するテスト種別１情報が取得できません。"; }

	if (!$ERROR) {
		$sql  = "SELECT * FROM " . T_MS_TEST_CATEGORY1 . " WHERE mk_flg='0'".
			" AND list_num < '".$m_list_num."' AND test_type_num='".$_POST['test_type_num']."' ORDER BY list_num DESC LIMIT 1;";
		if ($result = $cdb->query($sql)) {
			$list = $cdb->fetch_assoc($result);
			$c_test_category1_num = $list['test_category1_num'];
			$c_list_num = $list['list_num'];
		}
	}
	if (!$c_test_category1_num || !$c_list_num) { $ERROR[] = "移動されるテスト種別１情報が取得できません。"; }

	//対象データのlist_numを、対象データより1つ小さいlist_numに置き換える
	if (!$ERROR) {
		$INSERT_DATA['list_num'] 	= $c_list_num;
		$INSERT_DATA['upd_tts_id'] 	= $_SESSION['myid']['id'];
		$INSERT_DATA['upd_date'] 		= "now()";
		//upd start hirose 2018/11/29 すらら英単語
//		$where = " WHERE test_category1_num='".$m_test_category1_num."' LIMIT 1;";
		$where = " WHERE test_type_num='".$_POST['test_type_num']."' AND test_category1_num='".$m_test_category1_num."' LIMIT 1;";
		//upd end hirose 2018/11/29 すらら英単語
		$ERROR = $cdb->update(T_MS_TEST_CATEGORY1,$INSERT_DATA,$where);
	}

	//対象データより一つ小さいデータのlist_numを、対象データのlist_numに置き換える
	if (!$ERROR) {
		$INSERT_DATA['list_num'] 	= $m_list_num;
		$INSERT_DATA['upd_tts_id'] 	= $_SESSION['myid']['id'];
		$INSERT_DATA['upd_date'] 		= "now()";
		//upd start hirose 2018/11/29 すらら英単語
//		$where = " WHERE test_category1_num='".$c_test_category1_num."' LIMIT 1;";
		$where = " WHERE test_type_num='".$_POST['test_type_num']."' AND test_category1_num='".$c_test_category1_num."' LIMIT 1;";
		//upd end hirose 2018/11/29 すらら英単語
		$ERROR = $cdb->update(T_MS_TEST_CATEGORY1,$INSERT_DATA,$where);
	}
...
function down() {
	...
	if (!$ERROR) {
		$sql  = "SELECT * FROM " . T_MS_TEST_CATEGORY1 . " WHERE mk_flg='0'".
			" AND list_num >'".$m_list_num."' AND test_type_num='".$_POST['test_type_num']."' ORDER BY list_num LIMIT 1;";
		if ($result = $cdb->query($sql)) {
			$list = $cdb->fetch_assoc($result);
			$c_test_category1_num = $list['test_category1_num'];
			$c_list_num = $list['list_num'];
...
function csv_export() {
	...
	list($csv_line,$ERROR) = make_csv($L_CSV_COLUMN['test_word_test_category1'],1);
	if ($ERROR) { return $ERROR; }

	$filename = "test_word_test_category1_type".$_SESSION['sub_session']['test_type_num'].".csv";
...
function make_csv($L_CSV_COLUMN,$head_mode='1') {
	...
	foreach ($L_CSV_COLUMN as $key => $val) {
		if ($key == "test_type_num"){ continue; } //add kimura 2018/11/26 すらら英単語 _admin このカラムは扱わない
	...
	$sql  = "SELECT * FROM " . T_MS_TEST_CATEGORY1 .
		" WHERE mk_flg='0' AND test_type_num='".$_SESSION['sub_session']['test_type_num']."' ORDER BY list_num";
	if ($result = $cdb->query($sql)) {
		while ($list = $cdb->fetch_assoc($result)) {
			foreach ($L_CSV_COLUMN as $key => $val) {
			if ($key == "test_type_num"){ continue; } //add kimura 2018/11/26 すらら英単語 _admin このカラムは扱わない
...
function csv_import() {
	...
	for ($i = 1; $i < count($L_IMPORT_LINE);$i++) {
		...
		//レコードがあれば更新なければ新規
		//upd start hirose 2018/11/28 すらら英単語 list_numの修正
//		$sql = "SELECT test_category1_num FROM ". T_MS_TEST_CATEGORY1 .
		$sql = "SELECT test_category1_num,list_num FROM ". T_MS_TEST_CATEGORY1 .
		//upd end hirose 2018/11/28 すらら英単語 list_numの修正
			 // " WHERE test_category1_num='".$CHECK_DATA['test_category1_num']."' AND test_type_num = '".$CHECK_DATA['test_type_num']."' AND mk_flg='0' LIMIT 1;"; //del kimura 2018/11/26 すらら英単語 _admin
			 " WHERE test_category1_num = '".$CHECK_DATA['test_category1_num']."' AND test_type_num = '".$_SESSION['sub_session']['test_type_num']."' " //add kimura 2018/11/26 すらら英単語 _admin
				//. "AND mk_flg='0'" del hirose 2018/11/27 すらら英単語　既に削除されていた場合、プライマリーキーでインサートエラーになるため、条件除外
				. " LIMIT 1;"; //add kimura 2018/11/26 すらら英単語 _admin
		if ($result = $cdb->query($sql)) {
			$list = $cdb->fetch_assoc($result);
		}

		if ($list['test_category1_num']) {
			if($list['test_category1_num'] == $CHECK_DATA['test_category1_num']){
				//upd start hirose 2018/11/28 すらら英単語　list_numのチェック
//				$ins_mode = "upd";
				$return_error = check_list_num($_SESSION['sub_session']['test_type_num'],$CHECK_DATA['test_category1_num'],$CHECK_DATA['list_num'],$i);
				if($return_error){
					$ERROR[] = $return_error;
					continue;
				}else{
				...
				//upd end hirose 2018/11/28 すらら英単語　list_numのチェック
			} else {
				$ERROR[] = $i."行目のテスト種別１IDが不正なのでスキップしました。<br>";
				continue;
			}
		} else {
			//del start hirose すらら英単語 複合キーなので、test_category1_numがプライマリーにならなくてもよいため削除
			// $sql2 = "SELECT test_category1_num FROM ". T_MS_TEST_CATEGORY2 . //del kimura 2018/11/26 すらら英単語 _admin
//			$sql2 = "SELECT test_category1_num FROM ". T_MS_TEST_CATEGORY1 . //add kimura 2018/11/26 すらら英単語 _admin
//			 // " WHERE test_category1_num='".$CHECK_DATA['test_category1_num']."' AND mk_flg='0' LIMIT 1;"; //del kimura 2018/11/26 すらら英単語 _admin
//			 " WHERE test_category1_num = '".$CHECK_DATA['test_category1_num']."' AND test_type_num = '".$_SESSION['sub_session']['test_type_num']."' AND mk_flg='0' LIMIT 1;"; //add kimura 2018/11/26 すらら英単語 _admin
//			if ($result2 = $cdb->query($sql2)) {
//				$list2 = $cdb->fetch_assoc($result2);
//			}
//			if($list2['test_category1_num']){
//				$ERROR[] = $i."行目のテスト種別１IDはすでに使用されています。<br>";
//			} else {
//				$ins_mode = "add";
//			}
			//del start hirose すらら英単語 複合キーなので、test_category1_numがプライマリーにならなくてもよいため削除
		...
		// $DISP_SORT[$CHECK_DATA['test_category1_num']]++; //未使用なので削除 //del kimura 2018/11/26 すらら英単語 _admin
		...
		if ($ins_mode == "add") {
			if(!$INSERT_DATA['test_category1_num']){
				$sql = "SELECT MAX(test_category1_num) AS max_id FROM " . T_MS_TEST_CATEGORY1 . ";";
				if ($result = $cdb->query($sql)) {
					$list = $cdb->fetch_assoc($result);
				}
				if ($list['max_id']) {
					$INSERT_DATA['test_category1_num'] = $list['max_id'] + 1;
				} else {
					$INSERT_DATA['test_category1_num'] = 1;
				}
			}
			...
			$SYS_ERROR[$i] = $cdb->insert(T_MS_TEST_CATEGORY1,$INSERT_DATA);
			$new_test_cate1_num = $cdb->insert_id();
			$L_ID_NUM[$test_category1_num] = $new_test_cate1_num;
		} else {
			$INSERT_DATA['upd_tts_id'] 			= "System";
			$INSERT_DATA['upd_date'] 			= "now()";
			$INSERT_DATA['mk_flg']				= 0;//add hirose 2018/11/27 すらら英単語　削除されているものもあるので、mk_flgを0に直す
			// $where = " WHERE test_category1_num='".$INSERT_DATA['test_category1_num']."' LIMIT 1;"; //del kimura 2018/11/26 すらら英単語 _admin
			$where = " WHERE test_category1_num = '".$INSERT_DATA['test_category1_num']."'"; //add kimura 2018/11/26 すらら英単語 _admin
			$where.= " AND test_type_num = '".$_SESSION['sub_session']['test_type_num']."'"; //add kimura 2018/11/26 すらら英単語 _admin
			unset($INSERT_DATA['class_id']);
			unset($INSERT_DATA['test_category1_num']);
			$SYS_ERROR[$i] = $cdb->update(T_MS_TEST_CATEGORY1,$INSERT_DATA,$where);
		}
...
function check_data(&$CHECK_DATA,$ins_mode,$line_num) {
	...
	if ($CHECK_DATA['test_category1_num']){
		...
		if (preg_match("/[^0-9]/",$CHECK_DATA['test_category1_num'])) {
			$ERROR[] = $line_num."行目 テスト種別１IDは数字以外の指定はできません。";
		}elseif ($CHECK_DATA['test_category1_num'] < 1) {
			$ERROR[] = $line_num."行目 テスト種別１IDは0以下の指定はできません。";
		}
	} 
	if (!$CHECK_DATA['test_category1_name']) {
		$ERROR[] = $line_num."行目 テスト種別１名が未入力です。";
	}
	//add start kimura 2018/11/26 すらら英単語 _admin //同一テスト種別1名が存在しないことの確認
	else {
		$sql = "SELECT *";
		$sql.= " FROM " . T_MS_TEST_CATEGORY1;
		$sql.= " WHERE mk_flg='0'";
		$sql.= "  AND test_type_num = '".$_SESSION['sub_session']['test_type_num']."'";
		$sql.= "  AND test_category1_name = '".$CHECK_DATA['test_category1_name']."'";
		if($ins_mode == "upd"){ //更新時は自身以外に同一名がないか見る
			$sql.= "  AND test_category1_num != '".$CHECK_DATA['test_category1_num']."'";
		}
		$c = 0;
		if ($result = $cdb->query($sql)) {
		...
		if ($c > 0) {
			$ERROR[] = $line_num."行目 入力されたテスト種別１名は既に登録されております。";
		}
	}
	//add end   kimura 2018/11/26 すらら英単語 _admin

	//del start kimura 2018/11/26 すらら英単語 _admin このカラムは扱わない
	// 級IDチェック
 ...
 * @return string
 */
//add start hirose 2018/11/28 すらら英単語 list_numの修正
function check_list_num($test_type_num,$test_category1_num,$list_num,$i){
	// 分散DB接続オブジェクト
	$cdb = $GLOBALS['cdb'];
	...
	if ($result = $cdb->query($sql)) {
		while ($list = $cdb->fetch_assoc($result)) {
			$c++;
			//test_type_numで検索してるから、test_category1_numだけでプライマリー
			$list_array[$list['test_category1_num']] = $list['list_num'];
		}
	}else{
		$ERROR = $i . "行目のSQLエラーです";
	}
//	pre($list_array);
	if(!$ERROR){
		//同じtest_type_numの中でlist_numが使用されていなかったらアップデート
		if ($c == 0) {
			//アップデートしてよいので何もしない
		//同じtest_type_numの中でlist_numが1つあった場合、それが今チェックしているデータのものか確認
		} elseif ($c == 1) {
			if (isset($list_array[$test_category1_num])) {
				//アップデートしてよいので何もしない
			} else {
...
//add end hirose 2018/11/28 すらら英単語 list_numの修正
