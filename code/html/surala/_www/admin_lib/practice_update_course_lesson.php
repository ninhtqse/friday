...
function default_html($L_NAME,$ERROR) {
	...
	$sql  = "SELECT update_time FROM ".T_LESSON.
	...
	if ($local_time || $remote_time) {
		$submit_msg = $L_NAME['course_name']."のLesson情報を検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."のLesson情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
...
function update() {
	...
	if (!$ERROR && $COLUMN_NAME && $INSERT_VALUES) {
		$sql = "INSERT INTO ".T_LESSON." (".$COLUMN_NAME.") VALUES ".$INSERT_VALUES.";";
		if (!$connect_db->exec_query($sql)) {
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."のLesson情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
