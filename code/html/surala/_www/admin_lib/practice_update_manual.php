...
function default_html($L_NAME,$ERROR) {
	...
	//	ローカルサーバー
	unset($local_max);
	$local_dir = MATERIAL_MANUAL_DIR.$_POST['course_num']."/";
	...
	// -- リモートサーバー
	unset($remote_max);
	$remote_dir = REMOTE_MATERIAL_MANUAL_DIR;
	...
	if ($local_html || $remote_html) {
		$submit_msg = "マニュアルファイルを検証へアップしますがよろしいですか？";

		$html .= "マニュアルファイルをアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"$_SERVER[PHP_SELF]\" method=\"POST\" name=\"del_form_all\">\n";
		...
		$html .= "マニュアルファイルがアップロードされておりません。<br>\n";
	}
...
function update() {
	...
	//	マニュアルファイルアップロード
	$local_dir = BASE_DIR."/www".ereg_replace("^../","/",MATERIAL_MANUAL_DIR);
