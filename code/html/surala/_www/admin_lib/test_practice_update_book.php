...
// 		if($course_num == 15 || $course_num == 16){ continue; } // add 2018/05/18 yoshizawa 理科社会対応
	...
	if ($_SESSION['view_session']['course_num'] > 0) {
		if ($last_select_flg == 1) {
			...
			foreach($L_GKNN_LIST_TYPE2[$posted_course_num] as $key => $val) { // update $L_GKNN_LIST => $L_GKNN_LIST_TYPE1 2015/04/13 yoshizawa		// add karasawa 2020/12/22 定期テスト学年追加開発
				$selected = "";
...
function default_html($ERROR) {
	...
	//	情報取得クエリー
	$sql  = "SELECT MAX(ms_book.upd_date) AS upd_date FROM ".T_MS_BOOK." ms_book".
			" WHERE ms_book.publishing_id!='0'".
			" AND ms_book.course_num='".$_SESSION['view_session']['course_num']."'";
	if ($_SESSION['view_session']['gknn'] != "0" && $_SESSION['view_session']['gknn'] != "") {
		$sql .= " AND ms_book.gknn='".$_SESSION['view_session']['gknn']."'";
	}
	$sql .=	";";
	$sql_cnt  = "SELECT DISTINCT ms_book.* FROM ".T_MS_BOOK." ms_book".
				" WHERE ms_book.publishing_id!='0'".
				" AND ms_book.course_num='".$_SESSION['view_session']['course_num']."'";
	if ($_SESSION['view_session']['gknn'] != "0" && $_SESSION['view_session']['gknn'] != "") {
		$sql_cnt .= " AND ms_book.gknn='".$_SESSION['view_session']['gknn']."'";
	}
	$sql_cnt .=	";";
	...
	if ($local_time || $remote_time) {
		$submit_msg = "教科書情報を検証へアップしますがよろしいですか？";

		$html .= "教科書情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\">\n";
...
// 		$tbw_dbname = $connect_db->get_dbname();
	...
	if ($_SESSION['view_session']['gknn'] != "0" && $_SESSION['view_session']['gknn'] != "") {
		$sql .= " AND gknn='".$_SESSION['view_session']['gknn']."'";
	}
	$sql .= ";";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MS_BOOK, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	$sql  = "DELETE FROM ".T_MS_BOOK.
			" WHERE publishing_id!='0'".
			" AND course_num='".$_SESSION['view_session']['course_num']."'";
	if ($_SESSION['view_session']['gknn'] != "0" && $_SESSION['view_session']['gknn'] != "") {
		$sql .= " AND gknn='".$_SESSION['view_session']['gknn']."'";
	...
	//	検証バッチから検証webへ
	$send_data = " '".$_SESSION['view_session']['course_num']."' '".$_SESSION['view_session']['gknn']."'";
