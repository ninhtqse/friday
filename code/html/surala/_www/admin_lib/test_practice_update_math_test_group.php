...
function select_unit_view($ERROR) {
	...
	if (ACTION == "") {
		...
		$sql  = "SELECT * FROM ".T_MS_BOOK_GROUP." mtgm".
				 " WHERE mtgm.mk_flg='0'".
				// " AND mtgm.srvc_cd = 'STEST';"; // del 2020/09/14 cuong テスト標準化開発
				" AND mtgm.class_id > '';"; // add 2020/09/14 cuong テスト標準化開発
	...
	if ($result = $cdb->query($sql)) {
		while ($list=$cdb->fetch_assoc($result)) {
			...
			} elseif ($class_id != "" && ($test_group_id == "0" || $test_group_id == "")) {
				$PMUL[$class_id] = 1;
			}
		}
	}
	...
	foreach($L_MATH_TEST_CLASS as $key => $val) {
		...
		$selected = "";
		if (strval($_SESSION['view_session']['class_id']) == strval($key) && strval($_SESSION['view_session']['class_id']) != "" ) {
			if ( strval($_SESSION['view_session']['class_id']) !== "select"  ) {
				$selected = "selected";
			}
		}
...
function view_set_session() {
	...
	if ($_POST['class_id'] != "0" && $_POST['class_id'] != "" && $_POST['test_group_id'] != "") {
...
function default_html($ERROR) {
	...
	if ($_SESSION['view_session']['class_id'] != "" && $_SESSION['view_session']['class_id'] != "0") {
		$WHERE[] = " mbg.class_id='".$_SESSION['view_session']['class_id']."'";
	}
	if ($_SESSION['view_session']['test_group_id'] > 0) {
		$WHERE[] = " mbg.test_group_id='".$_SESSION['view_session']['test_group_id']."'";
	}
	$WHERE[] = " mbg.mk_flg='0'";
	// $WHERE[] = " mbg.srvc_cd = 'STEST'"; // del 2020/09/14 cuong テスト標準化開発
	$WHERE[] = " mbg.class_id > ''"; // add 2020/09/14 cuong テスト標準化開発
	if ($WHERE) {
	...
	if ($local_time || $remote_time) {
		$submit_msg = "数学検定グループ情報を検証へアップしますがよろしいですか？";

		$html .= "数学検定グループ情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\">\n";
...
function update(&$ERROR) {
	...
	//	検証バッチから検証webへ
	$send_data = " '".$_SESSION['view_session']['class_id']."' '".$_SESSION['view_session']['test_group_id']."'";
...
function update_end_html() {
	...
	$html  = "数学検定グループ情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
	$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\">\n";
