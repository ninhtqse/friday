 ...
 */
function action() {
//echo "<br>This is under construction ... test_practice_update_math_test_book_info.php<br>";	//test add okabe 2020/09/04 @@@
...
function default_html($ERROR) {
	...
	//	情報取得クエリー
	$sql  = "SELECT MAX(math_test_book_info.upd_date) AS upd_date FROM ".T_MATH_TEST_BOOK_INFO." math_test_book_info;";
	$sql_cnt  = "SELECT DISTINCT math_test_book_info.*  FROM ".T_MATH_TEST_BOOK_INFO." math_test_book_info;";
	...
	if ($local_time || $remote_time) {
		$submit_msg = "数学検定情報を検証へアップしますがよろしいですか？";

		$html .= "数学検定情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER['PHP_SELF']."\" method=\"POST\">\n";
...
function update(&$ERROR) {
	...
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MATH_TEST_BOOK_INFO, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	$sql  = "DELETE FROM ".T_MATH_TEST_BOOK_INFO.";";
	$DELETE_SQL[] = $sql;
