...
function default_html($L_NAME,$ERROR) {
	...
	$sql  = "SELECT update_time FROM ".T_REVIEW_SETTING.
	...
	if ($local_time || $remote_time) {
		$submit_msg = $L_NAME['course_name']."の診断復習情報を検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."の診断復習情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
...
function update() {
	...
	//	検証バッチDBデーター削除
	$sql  = "DELETE FROM ".T_REVIEW_SETTING.
			" WHERE course_num='".$_POST['course_num']."';";
	if (!$connect_db->exec_query($sql)) {
	...
	if ($COLUMN_NAME && $INSERT_VALUES) {
		$sql = "INSERT INTO ".T_REVIEW_SETTING." (".$COLUMN_NAME.") VALUES ".$INSERT_VALUES.";";
		if (!$connect_db->exec_query($sql)) {
	...
	//	テーブル最適化
	$sql = "OPTIMIZE TABLE ".T_REVIEW_SETTING.";";
	if (!$connect_db->exec_query($sql)) {
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."の診断復習情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
