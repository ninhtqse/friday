...
function default_html($L_NAME,$ERROR) {
	...
	//	NGワード基本情報
	$sql  = "SELECT max(upd_date) AS upd_date FROM ".T_NG_WORD.";";
	...
	if ($local_time || $remote_time) {
		$submit_msg = "NGワード情報を検証へアップしますがよろしいですか？";

		$html .= "NGワード情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
...
function update() {
	...
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_NG_WORD, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	$sql = "TRUNCATE TABLE ".T_NG_WORD.";";
	$DELETE_SQL[] = $sql;
