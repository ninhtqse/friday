...
function start() {
	...
	list($html,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT) = select_course();
	if (MODE == "add") {
		if (ACTION == "check") {
			if (!$ERROR) { $html .= check_html($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
			else { $html .= addform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
		} elseif (ACTION == "add") {
			if (!$ERROR) { $html .= unit_list($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
			else { $html .= addform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
		} else {
			$html .= addform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT);
		}
	} elseif (MODE == "詳細") {
		if (ACTION == "check") {
			if (!$ERROR) { $html .= check_html($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
			else { $html .= viewform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
		} elseif (ACTION == "change") {
			if (!$ERROR) { $html .= unit_list($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
			else { $html .= viewform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
		} else {
			$html .= viewform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT);
		}
	} elseif (MODE == "削除") {
		if (ACTION == "check") {
			if (!$ERROR) { $html .= check_html($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT); }
			...
			$html .= check_html($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT);
		}
	} else {
		if ($_POST[pk_course_num]&&$_POST[pk_stage_num]&&$_POST[pk_lesson_num]&&$_POST[pk_unit_num]) {
			$html .= unit_list($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT);
		}
	}
...
function select_course() {
	...
	if ($_POST[pk_stage_num]&&$max) {
		...
		if (!$max) {
			$pk_lesson_num_html .= "<option value=\"\">ステージ内にレッスンがありません。</option>\n";
		} else {
 ...
 * @return string HTML
 */
function unit_list($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT) {
	...
	if (!$_SESSION['authority'] || ($_SESSION['authority'] && array_search(MAIN."__".SUB."__add",$_SESSION['authority'])===FALSE)) {
		...
		if ($list['category_type'] != 3) {
			...
			$html .= "<input type=\"submit\" value=\"ブロック新規登録\">\n";
			$html .= "</form>\n";
		} else {
			$html .= "<br>\n";
	...
	$sql  = "SELECT * FROM ".T_PACKAGE_BLOCK.
			" WHERE mk_flg!='1' AND pk_unit_num='$_POST[pk_unit_num]' ORDER BY list_num;";
	if ($result = $cdb->query($sql)) {
		$max = $cdb->num_rows($result);
	}
	if (!$max) {
 ...
 * @return string HTML
 */
function addform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT) {
	...
	$make_html->set_file(PACKAGE_BLOCK_FORM);
...
//	$配列名['置換コメント名'] = array('コマンド名'=>'値','コマンド名'=>'値',....);
	...
	$INPUTS[BLOCK] = array('type'=>'text','name'=>'pk_block_name','value'=>$_POST['pk_block_name']);
 ...
 * @return string HTML
 */
function viewform($ERROR,$L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT) {
	...
	if ($action) {
		...
		$sql = "SELECT * FROM ".T_PACKAGE_BLOCK.
			" WHERE pk_block_num='$_POST[pk_block_num]' AND mk_flg!='1' LIMIT 1;";
	...
	if (!$pk_block_num) { $pk_block_num = "---"; }
	$INPUTS[LESSONNUM] = array('result'=>'plane','value'=>$pk_block_num);
	$INPUTS[COURSENAME] = array('result'=>'plane','value'=>$L_COURSE[$_POST[pk_course_num]]);
	$INPUTS[UNITGROUPNAME] = array('result'=>'plane','value'=>$L_STAGE[$_POST[pk_stage_num]]);
	$INPUTS[LESSON] = array('result'=>'plane','value'=>$L_LESSON[$_POST[pk_lesson_num]]);
	$INPUTS[UNIT] = array('result'=>'plane','value'=>$L_UNIT[$_POST[pk_unit_num]]);
	$INPUTS[BLOCK] = array('type'=>'text','name'=>'pk_block_name','value'=>$pk_block_name);
...
function check() {
	...
	if (!$_POST['pk_course_num']) { $ERROR[] = "登録するブロックのコース情報が確認できません。"; }
	if (!$_POST['pk_stage_num']) { $ERROR[] = "登録するブロックのステージ情報が確認できません。"; }
	if (!$_POST['pk_lesson_num']) { $ERROR[] = "登録するブロックのステージ情報が確認できません。"; }
	if (!$_POST['pk_unit_num']) { $ERROR[] = "登録するブロックのユニット情報が確認できません。"; }

	if (!$_POST['pk_block_name']) { $ERROR[] = "ブロック名が未入力です。"; }
	elseif (!$ERROR) {
		if ($mode == "add") {
			$sql  = "SELECT * FROM ".T_PACKAGE_BLOCK.
	...
	//	add ookawara 2012/05/30 start
		...
		$sql  = "SELECT count(*) AS count FROM ".T_PACKAGE_UNIT_LIST.
				...
				" AND pk_block_num='".$_POST['pk_block_num']."'".
				" AND mk_flg!='1';";
		if ($result = $cdb->query($sql)) {
			$list = $cdb->fetch_assoc($result);
			$count = $list['count'];
		...
		pakage_cat_check($ERROR, $_POST['pk_course_num'], $_POST['pk_stage_num'], $_POST['pk_lesson_num'], $_POST['pk_unit_num'], $_POST['pk_block_num']);
 ...
 * @return string HTML
 */
function check_html($L_COURSE,$L_STAGE,$L_LESSON,$L_UNIT) {
...
//	$配列名['置換コメント名'] = array('コマンド名'=>'値','コマンド名'=>'値',....);
	...
	$INPUTS[LESSON] = array('result'=>'plane','value'=>$L_LESSON[$_POST[pk_lesson_num]]);
	$INPUTS[UNIT] = array('result'=>'plane','value'=>$L_UNIT[$_POST[pk_unit_num]]);
	$INPUTS[BLOCK] = array('result'=>'plane','value'=>$pk_block_name);
...
function add() {
	...
	$ERROR = $cdb->insert(T_PACKAGE_BLOCK,$INSERT_DATA);

	if (!$ERROR) {
		$pk_block_num = $cdb->insert_id();
		$INSERT_DATA[list_num] = $pk_block_num;
		$where = " WHERE pk_block_num='$pk_block_num' LIMIT 1;";

		$ERROR = $cdb->update(T_PACKAGE_BLOCK,$INSERT_DATA,$where);
	}
...
function change() {
	...
	if (MODE == "詳細") {
		...
		$INSERT_DATA[pk_block_name] = $_POST[pk_block_name];
		...
		$INSERT_DATA[upd_date] = "now()";
		$INSERT_DATA[upd_tts_id] 		= $_SESSION['myid']['id'];
		$where = " WHERE pk_block_num='$_POST[pk_block_num]' LIMIT 1;";
...
function up() {
	...
	$sql  = "SELECT * FROM ".T_PACKAGE_BLOCK.
			" WHERE pk_block_num='$_POST[pk_block_num]' LIMIT 1;";
	if ($result = $cdb->query($sql)) {
		$list = $cdb->fetch_assoc($result);
	...
	if (!$m_pk_block_num || !$m_list_num) { $ERROR[] = "移動するブロック情報が取得できません。"; }

	if (!$ERROR) {
		$sql  = "SELECT * FROM ".T_PACKAGE_BLOCK.
				" WHERE mk_flg!='1' AND pk_unit_num='$m_pk_unit_num' AND list_num<'$m_list_num'" .
				" ORDER BY list_num DESC LIMIT 1;";
		if ($result = $cdb->query($sql)) {
			$list = $cdb->fetch_assoc($result);
			$c_pk_block_num = $list['pk_block_num'];
			$c_list_num = $list['list_num'];
		}
	}
	if (!$c_pk_block_num || !$c_list_num) { $ERROR[] = "移動されるブロック情報が取得できません。"; }
	...
	if (!$ERROR) {
		...
		$INSERT_DATA[upd_date] = "now()";
		$INSERT_DATA[upd_tts_id] 		= $_SESSION['myid']['id'];
		$where = " WHERE pk_block_num='$m_pk_block_num' LIMIT 1;";
	...
	if (!$ERROR) {
		...
		$INSERT_DATA[upd_date] = "now()";
		$INSERT_DATA[upd_tts_id] 		= $_SESSION['myid']['id'];
		$where = " WHERE pk_block_num='$c_pk_block_num' LIMIT 1;";
