...
function default_html($ERROR) {
	...
	//	ローカルサーバー
	$local_dir = MATERIAL_CHART_DIR."common/";
//	$local_dir = MATERIAL_CHART_DIR."common_test/";
	...
	// -- リモートサーバー
	$remote_dir = REMOTE_MATERIAL_CHART_DIR."common/";
	//$remote_dir = REMOTE_MATERIAL_CHART_DIR."common_test/";
	...
	if ($local_html || $remote_html) {
		$submit_msg = "体系図commonフォルダを検証へアップしますがよろしいですか？";

		$html .= "体系図commonフォルダをアップする場合は、対象のフォルダをチェック後、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER['PHP_SELF']."\" method=\"POST\" name=\"del_form_all\">\n";
		...
		$html .= "体系図commonフォルダがアップロードされておりません。<br>\n";
	}
...
function update() {
	...
	if($_POST['update_dir'] && $_POST['common_dir']){
		foreach ($_POST['update_dir'] as $key => $update_dir) {
			...
			//	体系図ファイルアップロード
			$remove_dir = KBAT_DIR.REMOTE_MATERIAL_CHART_DIR."common/".$update_dir;
			//$remove_dir = KBAT_DIR.REMOTE_MATERIAL_CHART_DIR."common_test/".$update_dir;
			$local_dir = BASE_DIR."/www".preg_replace("/^..\//","/",MATERIAL_CHART_DIR)."common/".$update_dir;
			//$local_dir = BASE_DIR."/www".preg_replace("/^..\//","/",MATERIAL_CHART_DIR)."common_test/".$update_dir;
			$remote_dir = KBAT_DIR.REMOTE_MATERIAL_CHART_DIR."common/".$update_dir_remote;
			//$remote_dir = KBAT_DIR.REMOTE_MATERIAL_CHART_DIR."common_test/".$update_dir_remote;
...
// echo "command2=".$command2."<br><br>";
			}elseif($file_type == 'd'){
				// 反映先のフォルダを削除してからローカルのレクチャーをアップする。
...
function update_end_html() {
	...
	$html  = "体系図commonフォルダのアップが完了致しました。<br>\n";
	$html .= "<br>\n";
