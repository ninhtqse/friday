...
function default_html($ERROR) {
	...
	//	情報取得クエリー
	$sql  = "SELECT MAX(ms_publishing.upd_date) AS upd_date FROM ".T_MS_PUBLISHING." ms_publishing;";
	$sql_cnt  = "SELECT DISTINCT ms_publishing.*  FROM ".T_MS_PUBLISHING." ms_publishing;";
	...
	if ($local_time || $remote_time) {
		$submit_msg = "出版社情報を検証へアップしますがよろしいですか？";

		$html .= "出版社情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\">\n";
...
function update(&$ERROR) {
	...
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MS_PUBLISHING, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	$sql  = "DELETE FROM ".T_MS_PUBLISHING.";";
	$DELETE_SQL[] = $sql;
	...
	//	テーブル最適化
	$sql = "OPTIMIZE TABLE ".T_MS_PUBLISHING.";";
	if (!$connect_db->exec_query($sql)) {
