 ...
 */
function action() {
//echo "<br>This is under construction ... test_practice_update_standard_category.php<br>";	//test add okabe 2020/09/04 @@@
...
function default_html($ERROR) {
	...
	//	情報取得クエリー
	$sql  = "SELECT MAX(ms_test_standard_category.upd_date) AS upd_date FROM ".T_MS_TEST_STANDARD_CATEGORY." ms_test_standard_category;";
	$sql_cnt  = "SELECT DISTINCT ms_test_standard_category.*  FROM ".T_MS_TEST_STANDARD_CATEGORY." ms_test_standard_category;";
	$sql2  = "SELECT MAX(test_standard_category_relation.upd_date) AS upd_date FROM ".T_TEST_STANDARD_CATEGORY_RELATION." test_standard_category_relation;";
	$sql_cnt2  = "SELECT DISTINCT test_standard_category_relation.*  FROM ".T_TEST_STANDARD_CATEGORY_RELATION." test_standard_category_relation;";
	...
	if ($result = $cdb->query($sql)) {
		$list = $cdb->fetch_assoc($result);
		$local_time2 = $list['upd_date'];
	}
	if ($result = $cdb->query($sql_cnt)) {
		$cnt = $cdb->num_rows($result);
	}
	if ($local_time) {
		$local_html2 = $local_time." (".$cnt.")";
	} else {
		$local_html2 = "データーがありません。";
	}
	...
	if ($result = $connect_db->query($sql)) {
		$list = $connect_db->fetch_assoc($result);
		$remote_time2 = $list['upd_date'];
	}
	if ($result = $connect_db->query($sql_cnt)) {
		$cnt = $connect_db->num_rows($result);
	}
	if ($remote_time) {
		$remote_html2 = $remote_time." (".$cnt.")";
	} else {
		$remote_html2 = "データーがありません。";
	}

	if ($local_time || $remote_time || $local_time2 || $remote_time2) {
		$submit_msg = "テスト標準化の情報を検証へアップしますがよろしいですか？";

		$html .= "テスト標準化の情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER['PHP_SELF']."\" method=\"POST\">\n";
		...
		$html .= "<td>テスト標準化:".T_MS_TEST_STANDARD_CATEGORY."</td>\n";
		$html .= "<td>\n";
		$html .= $local_html;
		$html .= "</td>\n";
		$html .= "<td>\n";
		...
		$html .= "<tr valign=\"top\" bgcolor=\"#ffffff\" align=\"center\">\n";
		$html .= "<td>テスト標準化:".T_TEST_STANDARD_CATEGORY_RELATION."</td>\n";
		$html .= "<td>\n";
		$html .= $local_html2;
		$html .= "</td>\n";
		$html .= "<td>\n";
...
function update(&$ERROR) {
	...
	//	更新情報クエリー
	//ms_test_standard_category
	$sql  = "SELECT * FROM ".T_MS_TEST_STANDARD_CATEGORY.";";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MS_TEST_STANDARD_CATEGORY, $INSERT_NAME, $INSERT_VALUE);
	}
	//test_standard_category_relation
	$sql  = "SELECT * FROM ".T_TEST_STANDARD_CATEGORY_RELATION.";";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_TEST_STANDARD_CATEGORY_RELATION, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	//ms_test_standard_category
	$sql  = "DELETE FROM ".T_MS_TEST_STANDARD_CATEGORY.";";
	$DELETE_SQL[] = $sql;
	//test_standard_category_relation
	$sql  = "DELETE FROM ".T_TEST_STANDARD_CATEGORY_RELATION.";";
	$DELETE_SQL[] = $sql;
	...
	//	テーブル最適化
	$sql = "OPTIMIZE TABLE ".T_MS_TEST_TYPE.";";
	if (!$connect_db->exec_query($sql)) {
