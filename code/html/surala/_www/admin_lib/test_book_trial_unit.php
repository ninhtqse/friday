...
function select_unit_view() {
	...
	$html .= "</td>\n";
	$html .= "<td>\n";
	$html .= "<select name=\"gknn\" onchange=\"submit();\">".$gknn_html."</select>\n";
	$html .= "</td>\n";
	$html .= "<td>\n";
	...
	if (!$_SESSION['t_practice']['course_num'] || !$_SESSION['t_practice']['gknn']) {
		$html .= "<br>\n";
		$html .= "単元を設定するコース、学年を選択してください。<br>\n";
	} elseif (!$_SESSION['t_practice']['book_id']) {
		$html .= "<br>\n";
		$html .= "単元を設定するマスタを選択してください。<br>\n";
	}
...
function book_unit_list($ERROR) {
	...
	$html .= "<br>\n";
	$html .= "<span style=\"font-weight:bold;\">マスタ ： ".$_SESSION['t_practice']['book_name']."</span>";

	$html .= "<br>\n";
	$html .= "インポートする場合は、マスタ単元csvファイル（S-JIS）を指定しCSVインポートボタンを押してください。<br>\n";
...
function addform($ERROR) {
	...
	$make_html->set_file(TEST_BOOK_TRIAL_UNIT_FORM);
...
function check() {
	...
	if (!$_POST['book_unit_name']) { $ERROR[] = "マスタ単元名が未入力です。"; }
...
/**
 * マスタ単元　マスタ単元とLMS単元の紐付け登録処理(未使用 2012/07/27)
 *
 * AC:[A]管理者 UC1:[M01]Core管理機能.
 *
 * @author Azet
...
function csv_export() {
	...
	list($csv_line,$ERROR) = make_csv($L_CSV_COLUMN['book_trial_unit'],1,1);
	if ($ERROR) { return $ERROR; }

	$filename = "ms_trial_unit_".$_SESSION['t_practice']['book_id'].".csv";
...
//pre($PARENT_BOOK_UNIT_NUM);
		...
		if ($list['book_id']) {
			if ($list['book_id'] == $CHECK_DATA['book_id']) {
				...
				$ERROR[] = $i."行目のマスタ単元IDかマスタIDが不正なのでスキップしました。<br>";
				continue;
			}
		} else { $ins_mode = "add"; }
...
function check_data(&$CHECK_DATA,$ins_mode,$line_num) {
	...
	if (!$CHECK_DATA['book_unit_id']) {
		...
		if (preg_match("/[^0-9]/",$CHECK_DATA['book_unit_id'])) {
			$ERROR[] = $line_num."行目 マスタ単元IDは数字以外の指定はできません。";
		} elseif ($CHECK_DATA['book_unit_id'] < 1) {
			$ERROR[] = $line_num."行目 マスタ単元IDは1以下の指定はできません。";
		}
	}
	if (!$CHECK_DATA['book_id']) { $ERROR[] = $line_num."行目 マスタIDが未入力です。"; }
	else {
		if ($CHECK_DATA['book_id'] != $_SESSION['t_practice']['book_id']) {
			$ERROR[] = $line_num."行目 マスタIDは選択マスタのID「".$_SESSION['t_practice']['book_id']."」以外の指定はできません。";
		}
	}
	if (!$CHECK_DATA['book_unit_name']) { $ERROR[] = $line_num."行目 マスタ単元名が未入力です。"; }
