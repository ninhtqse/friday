...
/*
	基本情報
		本番バッチではms_test_default_problemの以下の値でグルーピングしてアップデートする。
			default_test_num =0	固定
			course_num='1'					// アップデート指定画面のコースのリストボックスの値を利用（必須指定）
			gknn='J2'						// アップデート指定画面の学年のリストボックスの値を利用（任意指定）
			term_bnr_ccd='C000000001'		// アップデート指定画面のテスト時期のリストボックスの値を利用（任意指定）
			term_kmk_ccd='11'				// アップデート指定画面のテスト時期のリストボックスの値を利用（任意指定）

		変更テーブルは5テーブルとなり以下の手順で処理をしていく
			(問題)	※画像情報もここで取得する。
			コピー先の「ms_test_default_problem」のの該当する情報を削除
			コピー元の「ms_test_default_problem」の該当する情報を作成し、コピー先にREPLACE INTOで更新する
			(問題マスタ)
			...
			コピー元の「book_unit_test_problem」の該当する情報を作成し、REPLACE INTOで更新する
			(LMS単元)
			...
			コピー元の「upnavi_section_problem」の該当する情報を作成し、コピー先にインサートする（PKが無いのでINSERT処理を実行）
...
function select_unit_view($ERROR) {
	...
	// global $L_GKNN_LIST_TYPE1; // update $L_GKNN_LIST => $L_GKNN_LIST_TYPE1 2015/04/13 yoshizawa
	global $L_GKNN_LIST_TYPE2; //add karasawa 2020/12/22 定期テスト学年追加開発
	// global $L_WRITE_TYPE;// del hirose 2020/11/06 テスト標準化開発 定期テスト
...
// 		if($course_num == 15 || $course_num == 16){ continue; } // add 2018/05/18 yoshizawa 理科社会対応
	...
	if ($_SESSION['view_session']['gknn'] != "0" && $_SESSION['view_session']['gknn'] != "") {
		if ($last_select_flg == 1) {
			$core_code_html .= "<option value=\"\">アップデート中の為選択出来ません</option>\n";
		} else {
			$core_code_count = 0;
			$sql  = "SELECT * FROM ".T_MS_CORE_CODE.
					" WHERE mk_flg='0';";
			if ($result = $cdb->query($sql)) {
				$core_code_count = $cdb->num_rows($result);
			}
			if (!$core_code_count) {
				$html .= "テスト時期が存在しません。設定してからご利用下さい。";
				return $html;
			}
			...
			while ($list = $cdb->fetch_assoc($result)) {
				...
				if ($_SESSION['view_session']['core_code'] == $value) {
					$selected = "selected";
				}
				$core_code_html .= "<option value=\"".$value."\" ".$selected.">".$name."</option>\n";
			}
		}
	} else {
		$core_code_html .= "<option value=\"0\">--------</option>\n";
	}
	...
	$html .= "</td>\n";
	$html .= "<td>\n";
	$html .= "<select name=\"core_code\" onchange=\"submit();\">\n".$core_code_html."</select>\n";
	$html .= "</td>\n";
	$html .= "</tr>\n";
	$html .= "</table>\n";
	$html .= "</form>\n";
...
/**
 * コース、学年、テスト時期、出版社、教科書、単元選択セッションセット
 *
 * AC:[A]管理者 UC1:[L06]勉強をする.
 *
 * @author Azet
...
function view_set_session() {
	...
	$course_num = $_SESSION['view_session']['course_num'];
	$gknn = $_SESSION['view_session']['gknn'];
	$core_code = $_SESSION['view_session']['core_code'];
	...
	if ($_POST['gknn'] == $gknn && $_POST['core_code'] != "") {
		$_SESSION['view_session']['core_code'] = $_POST['core_code'];
	} else {
		return;
	}
...
function default_html($ERROR) {
	...
	//	絞込クエリー情報
	list($term_bnr_ccd,$term_kmk_ccd) = split("_", $_SESSION['view_session']['core_code']);
	...
	if ($_SESSION['view_session']['core_code'] != "0" && $_SESSION['view_session']['core_code'] != "") {
		$where .= " AND mtdp.term_bnr_ccd='".$term_bnr_ccd."'";
		$where .= " AND mtdp.term_kmk_ccd='".$term_kmk_ccd."'";
	}

	// update oda 2014/10/16 課題要望一覧No352 以下のJOINをLEFTからINNERに変更
	...
	$sql_cnt  = "SELECT mtdp.* FROM ".T_MS_TEST_DEFAULT_PROBLEM." mtdp".
				" WHERE mtdp.default_test_num='0'".
				$where.";";

	//	ms_test_problem
	$sql2 = "SELECT MAX(mtp.upd_date) AS upd_date FROM ".T_MS_TEST_PROBLEM." mtp".
			" INNER JOIN ".T_MS_TEST_DEFAULT_PROBLEM." mtdp ON mtdp.problem_num=mtp.problem_num".
	...
	$sql3_2 = "SELECT MAX(butp.upd_date) AS upd_date FROM ".T_BOOK_UNIT_TEST_PROBLEM." butp".
			" INNER JOIN ".T_MS_TEST_DEFAULT_PROBLEM." mtdp ON mtdp.problem_num=butp.problem_num".
				" AND mtdp.default_test_num='0'".
				" AND mtdp.problem_table_type='2'".
				$where.
			" WHERE mtdp.problem_num>0".
	...
	$sql4_2 = "SELECT MAX(bulu.upd_date) AS upd_date FROM ".T_BOOK_UNIT_LMS_UNIT." bulu".
			" INNER JOIN ".T_MS_TEST_DEFAULT_PROBLEM." mtdp ON mtdp.problem_num=bulu.problem_num".
				" AND mtdp.default_test_num='0'".
				" AND mtdp.problem_table_type='2'".
				$where.
			" WHERE mtdp.problem_num>0".
...
//echo "sql4_2=".$sql4_2."<br>";
//echo "sql4_1=".$sql4_1."<br>";
//echo "sql4_2_cnt=".$sql4_2_cnt."<br>";
//echo "sql4_1_cnt=".$sql4_1_cnt."<br>";

	//	upnavi_section_problem
	$sql5_2 = "SELECT MAX(usp.upd_date) AS upd_date FROM ".T_UPNAVI_SECTION_PROBLEM." usp".
			" INNER JOIN ".T_MS_TEST_DEFAULT_PROBLEM." mtdp ON mtdp.problem_num=usp.problem_num".
				" AND mtdp.default_test_num='0'".
				" AND mtdp.problem_table_type='2'".
				$where.
			" WHERE mtdp.problem_num>0".
...
*/
	...
	if ($dsp_flg == 1) {
		$submit_msg = "問題設定(定期テスト)情報を検証へアップしますがよろしいですか？";

		$html .= "問題設定(定期テスト)情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		...
		$html .= "<td>問題管理：".T_MS_TEST_DEFAULT_PROBLEM."</td>\n";
		$html .= "<td>\n";
		$html .= $local_html;
		$html .= "</td>\n";
		$html .= "<td>\n";
		...
		$html .= "問題設定(定期テスト)情報が設定されておりません。<br>\n";
...
function update(&$ERROR) {
	...
	// update start oda 2014/10/16 課題要望一覧No352
	...
	$sql  = "SELECT DISTINCT mtdp.* FROM ".T_MS_TEST_DEFAULT_PROBLEM." mtdp ".
			...
			$where.";";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MS_TEST_DEFAULT_PROBLEM, $REPLACE_NAME, $REPLACE_VALUE);
	}
	...
	$sql  = "SELECT DISTINCT butp.* FROM ".T_BOOK_UNIT_TEST_PROBLEM." butp ".
			...
			";";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_BOOK_UNIT_TEST_PROBLEM, $REPLACE_NAME, $REPLACE_VALUE);
	}
	...
	// update end oda 2014/10/16 課題要望一覧No352
	...
	//$send_data = " '".$_SESSION['view_session']['course_num']."' '".$gknn."' '".$core_code."' '".$fileup."' '".$update_num."'";
	//$command = "ssh suralacore01@srlbtw21 ./TESTCONTENTSUP.cgi '2' 'test_".MODE."'".$send_data;
	$send_data = " '".$_SESSION['view_session']['course_num']."' '".$gknn."' '".$core_code."' '".$fileup."' '".$update_num."' '".$_SESSION['myid']['id']."'";
	...
	exec($command);	//	add ookawara 2012/08/21
...
// 	}
// 	if ($_SESSION['view_session']['core_code'] != "0" && $_SESSION['view_session']['core_code'] != "") {
// 		$sql .= " AND lesson_num='".$_SESSION['view_session']['core_code']."'";
// 	} else {
...
// 		}
// 		if ($_SESSION['view_session']['core_code'] != "0" && $_SESSION['view_session']['core_code'] != "") {
// 			$where .= " AND lesson_num='".$_SESSION['view_session']['core_code']."'";
// 		}
...
// 		}
// 		if ($_SESSION['view_session']['core_code'] != "0" && $_SESSION['view_session']['core_code'] != "") {
// 			$INSERT_DATA['lesson_num'] = $_SESSION['view_session']['core_code'];
// 		}
// 		if ($_SESSION['view_session']['fileup'] == "1") {
// 			$INSERT_DATA['unit_num'] = $_SESSION['view_session']['fileup'];
// 		}
// 		$INSERT_DATA['send_data'] = $send_data_log;
// 		$INSERT_DATA['regist_time'] = "now()";
// 		$INSERT_DATA['state'] = 1;
...
function update_end_html() {
	...
	$html  = "問題設定(定期テスト)情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
	$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\">\n";
