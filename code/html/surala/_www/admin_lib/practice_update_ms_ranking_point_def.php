 ...
 * 	サブプログラム	ランキングマスターデフォルト情報アップデート
 *
 * @author Azet
 */
...
function default_html($L_NAME,$ERROR) {
	...
	if ($local_time || $remote_time) {
		$submit_msg = "ランキングマスターデフォルト情報を検証へアップしますがよろしいですか？";

		$html .= "ランキングマスターデフォルト情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
		...
		$html .= "ランキングマスターデフォルト情報が設定されておりません。<br>\n";
	}
...
function update() {
	...
	//	検証バッチDBデーター削除クエリー
	$sql  = "DELETE FROM ".T_MS_RANKING_POINT.
			" WHERE course_num='0';";
	$DELETE_SQL[] = $sql;
...
function update_end_html($L_NAME) {
	...
	$html  = "ランキングマスターデフォルト情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
