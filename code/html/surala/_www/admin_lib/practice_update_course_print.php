...
function default_html($L_NAME,$ERROR) {
	...
	//	ローカルサーバー
	unset($local_max);
	$local_dir = MATERIAL_PRINT_DIR.$_POST['course_num']."/";
	...
	// -- リモートサーバー
	unset($remote_max);
	$remote_dir = REMOTE_MATERIAL_PRINT_DIR.$_POST['course_num']."/";
	...
	if ($local_html || $remote_html) {
		$submit_msg = $L_NAME['course_name']."のまとめプリントファイルを検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."のまとめプリントファイルをアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"$_SERVER[PHP_SELF]\" method=\"POST\" name=\"del_form_all\">\n";
		...
		$html .= "まとめプリントファイルがアップロードされておりません。<br>\n";
	}
...
function update() {
	...
	//	まとめプリントファイルアップロード
	$local_dir = BASE_DIR."/www".ereg_replace("^../","/",MATERIAL_PRINT_DIR).$_POST['course_num'];
	$remote_dir = KBAT_DIR.REMOTE_MATERIAL_PRINT_DIR;
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."のまとめプリントファイルのアップが完了致しました。<br>\n";
	$html .= "<br>\n";
