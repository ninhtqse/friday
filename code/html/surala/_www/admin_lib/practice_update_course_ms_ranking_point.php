 ...
 * ベンチャー・リンク　すらら
 *
 * プラクティスランキングマスター管理　プラクティスアップデートプログラム
 * 	サブプログラム	ランキングマスター情報アップデート
 *
 * @author Azet
 */
...
function default_html($L_NAME,$ERROR) {
	...
	$sql  = "SELECT max(upd_date) AS upd_date FROM ".T_MS_RANKING_POINT.
	...
	if ($local_time || $remote_time) {
		$submit_msg = $L_NAME['course_name']."のランキングマスター情報を検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."のランキングマスター情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
		...
		$html .= "ランキングマスター情報が設定されておりません。<br>\n";
	}
...
function update() {
	...
	$sql  = "SELECT * FROM ".T_MS_RANKING_POINT.
			" WHERE course_num='".$_POST['course_num']."';";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MS_RANKING_POINT, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	$sql = "DELETE FROM ".T_MS_RANKING_POINT." WHERE course_num='".$_POST['course_num']."';";
	$DELETE_SQL[] = $sql;
	...
	//	テーブル最適化
	$sql = "OPTIMIZE TABLE ".T_MS_RANKING_POINT.";";
	if (!$connect_db->exec_query($sql)) {
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."のランキングマスター情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
