...
function default_html($L_NAME,$ERROR) {
	...
	$sql  = "SELECT update_time FROM ".T_COURSE.
	...
	if ($local_time || $remote_time) {
		$submit_msg = $L_NAME['course_name']."のコース情報を検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."のコース情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
...
function update() {
	...
	$sql  = "SELECT * FROM ".T_COURSE.
			" WHERE course_num='".$_POST['course_num']."';";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_COURSE, $INSERT_NAME, $INSERT_VALUE);
	}
	...
	//	検証バッチDBデーター削除クエリー
	$sql = "DELETE FROM ".T_COURSE." WHERE course_num='".$_POST['course_num']."';";
	$DELETE_SQL[] = $sql;
	//kaopiz 2020/11/09 CR admin external start
	$sql_delte_exteral = "DELETE FROM external_course WHERE course_num='".$_POST['course_num']."';";
	$DELETE_SQL[] = $sql_delte_exteral;
	//kaopiz 2020/11/09 CR admin external end
	...
	//kaopiz 2020/11/09 CR admin external start
	if (count($INSERT_NAME_EXTERNAL) && count($INSERT_VALUE_EXTERNAL)) {
		foreach ($INSERT_NAME_EXTERNAL AS $table_name => $insert_name) {
			if ($INSERT_VALUE_EXTERNAL[$table_name]) {
				foreach ($INSERT_VALUE_EXTERNAL[$table_name] AS $values) {
					$sql  = "INSERT INTO ".$table_name.
						" (".$insert_name.") ".
						" VALUES".$values.";";
					if (!$connect_db->exec_query($sql)) {
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."のコース情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
