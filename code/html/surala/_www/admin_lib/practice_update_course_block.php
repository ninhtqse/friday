...
function default_html($L_NAME,$ERROR) {
	...
	global $L_WRITE_TYPE,$L_DISPLAY,$L_UNIT_TYPE;
	...
	$sql  = "SELECT update_time FROM ".T_BLOCK.
...
// 「ブロック」から「ドリル」へ表記統一　2009/11/02 hirano
		$submit_msg = $L_NAME['course_name']."のドリル情報を検証へアップしますがよろしいですか？";

// 「ブロック」から「ドリル」へ表記統一　2009/11/02 hirano
		$html .= $L_NAME['course_name']."のドリル情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
...
function update() {
	...
	if ($COLUMN_NAME && $INSERT_VALUES) {
		$sql = "INSERT INTO ".T_BLOCK." (".$COLUMN_NAME.") VALUES ".$INSERT_VALUES.";";
		if (!$connect_db->exec_query($sql)) {
	...
	//	検証バッチから検証webへ
	// $command = "ssh suralacore01@srlbtw21 ./CONTENTSUP.cgi '2' '".MODE."' '".$_POST['course_num']."'"; // del 2018/03/19 yoshizawa AWSプラクティスアップデート
	$command = "/usr/bin/php ".BASE_DIR."/_www/batch/CONTENTSUP.cgi '2' '".MODE."' '".$_POST['course_num']."'"; // add 2018/03/19 yoshizawa AWSプラクティスアップデート
...
// 「ブロック」から「ドリル」へ表記統一　2009/11/02 hirano
	$html  = $L_NAME['course_name']."のドリル情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
