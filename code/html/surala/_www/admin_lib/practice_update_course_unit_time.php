...
function default_html($L_NAME,$ERROR) {
	...
	$sql  = "SELECT upd_date FROM ".T_PACKAGE_STANDARD_TIME.
	...
	if ($local_time || $remote_time) {
		$submit_msg = $L_NAME['course_name']."のユニット解答目安時間情報を検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."のユニット解答目安時間情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
		...
		$html .= "ユニット解答目安時間情報が設定されておりません。<br>\n";
	}
...
function update() {
	...
	//	データーベース更新
	//	ローカル情報取得
	$sql  = "SELECT * FROM ".T_PACKAGE_STANDARD_TIME.
			" WHERE course_num='".$_POST['course_num']."';";
	if ($result = $cdb->query($sql)) {
		$flag = 0;
		$i = 0;
	...
	//	検証バッチDBデーター削除
	$sql  = "DELETE FROM ".T_PACKAGE_STANDARD_TIME.
			" WHERE course_num='".$_POST['course_num']."';";
	if (!$connect_db->exec_query($sql)) {
	...
	if ($insert_name && count($INSERT_VALUE)) {
		foreach ($INSERT_VALUE AS $values) {
			$sql  = "INSERT INTO ".T_PACKAGE_STANDARD_TIME.
					" (".$insert_name.") ".
					" VALUES".$values.";";
			if (!$connect_db->exec_query($sql)) {
	...
	//	テーブル最適化
	$sql = "OPTIMIZE TABLE ".T_PACKAGE_STANDARD_TIME.";";
	if (!$connect_db->exec_query($sql)) {
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."のユニット解答目安時間情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
