...
function make_default_html($ERROR) {
	...
	$block_num = block_num;
	unset($count);
	$sql  = "SELECT problem_num, again, success FROM ".T_CHECK_STUDY_RECODE.
...
//			" AND DATE(regist_time)=CURDATE()".
	...
	//	最終学習問題表示番号取得
	if ($last_problem_num) {
		$sql  = "SELECT display_problem_num, sub_display_problem_num, clear_number FROM ".T_PROBLEM.
		...
		if ($result = $cdb->query($sql)) {
			...
			$last_sub_display_problem_num = $list['sub_display_problem_num'];
	...
	unset($problem_num);
	//	最終解答問題終了チェック
	if ($last_sub_display_problem_num) {	//	サブ問題
		if ($last_success != 1 && $last_again < $last_clear_number) {	//	復活
			$_SESSION['diagnostic']['parents_display_problem_num'] = $last_sub_display_problem_num;
			$_SESSION['record']['again'] = $last_again;
			$problem_num = $last_problem_num;
		} else {	//	次へ
			$count = 0;
			$sql  = "SELECT count(*) AS count FROM ".T_PROBLEM.
					" WHERE course_num='".$_SESSION['course']['course_num']."'".
					" AND block_num='".$block_num."'".
					" AND display_problem_num>'".$last_display_problem_num."'" .
					" AND problem_type='2'".
					" AND sub_display_problem_num='".$last_sub_display_problem_num."'" .
					" AND display='1'".
					" AND state!='1';";
			if (CHECK_MODE == 1) { $_SESSION[SQL][] = __FILE__ . "(" .__LINE__ . ")::" .$sql; }
			if ($result = $cdb->query($sql)) {
			...
			if ($count > 0) {
				...
				$last_display_problem_num = $last_sub_display_problem_num;
				unset($_SESSION['diagnostic']['parents_display_problem_num']);
			}
		}
	} else { //	基本問題
		if ($last_success != 1 && $last_again < $last_clear_number) {	//	復活
	...
	if (!$problem_num) {	//	次の問題に移行
		if ($_SESSION['diagnostic']['parents_display_problem_num']) {	//	不正解の親問題番号の設定がされていたら
			//	問題情報取得
			$sql  = "SELECT problem_num, display_problem_num, problem_type, sub_display_problem_num, form_type,".
					...
					" AND display_problem_num>'".$last_display_problem_num."'".
					" AND problem_type='2'" .
					" AND sub_display_problem_num='".$_SESSION['diagnostic']['parents_display_problem_num']."'" .
					" AND display='1'".
					" AND state!='1'".
		...
		if ($_SESSION['diagnostic']['parents_display_problem_num']) {	//	不正解の親問題番号の設定がされていたら
			...
			$sql  = "SELECT problem_num, display_problem_num, problem_type, sub_display_problem_num, form_type,".
					...
					" AND display='1'".
					" AND state!='1'".
					" ORDER BY sub_display_problem_num".
					" LIMIT 1;";
			if (CHECK_MODE == 1) { $_SESSION[SQL][] = __FILE__ . "(" .__LINE__ . ")::" .$sql; }
			if ($result = $cdb->query($sql)) {
				$PROBLEM_LIST = $list = $cdb->fetch_assoc($result);
	...
	if (($last_display_problem_num || $last_sub_display_problem_num) && !$problem_num) {
...
//		$ERROR = auto_difficulty2($_SESSION['record']['problem_num'],$_SESSION['record']['success']);
	...
	$block_num = block_num;
	if ($success != 1) {
		if (!$_SESSION['diagnostic']['parents_display_problem_num']) {	//	診断サブ問題で無かったら
			if ($_SESSION['record']['again'] >= correct_number) {
				...
				$sql  = "SELECT count(*) AS count FROM ".T_PROBLEM.
						" WHERE course_num='".$_SESSION['course']['course_num']."'".
						" AND block_num='".$block_num."'".
						" AND sub_display_problem_num='".$display_problem_num."'".
						" AND display='1'".
						" AND state!='1';";
				if (CHECK_MODE == 1) { $_SESSION[SQL][] = __FILE__ . "(" .__LINE__ . ")::" .$sql; }
				if ($result = $cdb->query($sql)) {
				...
				if ($count > 0) { $_SESSION['diagnostic']['parents_display_problem_num'] = $display_problem_num; }
			}
		}
	}
	...
	//	最終問題クリアーチェック
	$display_problem_num = display_problem_num;
	$sub_display_problem_num = sub_display_problem_num;
	if ($sub_display_problem_num > 0) {
		//	サブ問題の最後の問題番号取得
		$sql  = "SELECT sub_display_problem_num FROM ".T_PROBLEM.
				" WHERE course_num='".$_SESSION['course']['course_num']."'".
				" AND block_num='".$block_num."'".
				" AND display_problem_num='".$display_problem_num."'" .
				" AND problem_type='2'" .
				" AND display='1'".
				" AND state!='1'".
				" ORDER BY sub_display_problem_num DESC".
				" LIMIT 1;";
		if (CHECK_MODE == 1) { $_SESSION[SQL][] = __FILE__ . "(" .__LINE__ . ")::" .$sql; }
		if ($result = $cdb->query($sql)) {
			$list = $cdb->fetch_assoc($result);
		...
		//	サブ問題の最後の問題を呼び込み問題番号と一致していたら終わり
		if ($sub_display_problem_num == $last_sub_display_problem_num) {
			unset($_SESSION['diagnostic']['parents_display_problem_num']);
		}
	}

	if (!$_SESSION['diagnostic']['parents_display_problem_num']) {
		//	基本問題の最後の問題番号取得
...
function send_page_html() {
	...
	if ($_SESSION['record']['success'] == 1) {	//	正解
		...
		//	ヒント表示	サブ問題時
		if (sub_display_problem_num > 0 && hint_number > 0 && $_SESSION['record']['again'] >= hint_number && $_SESSION['record']['again'] < correct_number) {
			$hint = $display_problem->hint_check();
		}

		//	解答を見るボタン	サブ問題時
		if (sub_display_problem_num > 0 && correct_number > 0 && $_SESSION['record']['again'] >= correct_number) {
			...
			$form_end.= "</form>\n";
			$display_timerstartup = 1;
		} elseif (sub_display_problem_num < 1 && correct_number > 0 && $_SESSION['record']['again'] >= correct_number) {
			$display_timerstartup = 1;
		}
