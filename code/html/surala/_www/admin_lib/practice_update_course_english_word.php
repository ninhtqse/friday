...
function default_html($L_NAME,$ERROR) {
	...
	//	english_word
	$sql  = "SELECT MAX(upd_date) AS upd_date, count(*) AS cnt FROM ".T_ENGLISH_WORD.
			" WHERE course_num='".$_POST['course_num']."';";

	//	english_word_problem
	$sql2 = "SELECT MAX(upd_date) AS upd_date, count(*) AS cnt FROM ".T_ENGLISH_WORD_PROBLEM.
			" WHERE course_num='".$_POST['course_num']."';";
	...
	$set_dir = REMOTE_MATERIAL_WORD_IMG_DIR.$_POST['course_num']."/";
	//	ローカル
	...
	$set_dir = REMOTE_MATERIAL_WORD_VOICE_DIR.$_POST['course_num']."/";
	//	ローカル
	...
	if ($local_time || $remote_time) {
		$submit_msg = $L_NAME['course_name']."のワード情報を検証へアップしますがよろしいですか？";

		$html .= $L_NAME['course_name']."のワード情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
...
function update() {
	...
	//	検証バッチDBデーター削除クエリー
	//	english_word
	$sql  = "DELETE FROM ".T_ENGLISH_WORD.
			" WHERE course_num='".$_POST['course_num']."';";
	$DELETE_SQL['english_word'] = $sql;

	//	english_word
	$sql  = "DELETE FROM ".T_ENGLISH_WORD_PROBLEM.
			" WHERE course_num='".$_POST['course_num']."';";
	$DELETE_SQL['english_word_problem'] = $sql;
	...
	if ($_POST['fileup'] == 1) {
		...
		$local_dir = BASE_DIR.REMOTE_MATERIAL_WORD_IMG_DIR.$_POST['course_num']."/";
		$remote_dir = KBAT_DIR.REMOTE_MATERIAL_WORD_IMG_DIR;
...
//echo "command<>".$command."<br>\n";
		...
		$local_dir = BASE_DIR.REMOTE_MATERIAL_WORD_VOICE_DIR.$_POST['course_num']."/";
		$remote_dir = KBAT_DIR.REMOTE_MATERIAL_WORD_VOICE_DIR;
		// $command = "scp -rp ".$local_dir." suralacore01@srlbtw21:".$remote_dir; // del 2018/03/20 yoshizawa AWSプラクティスアップデート
		$command = "cp -rp ".$local_dir." ".$remote_dir; // add 2018/03/20 yoshizawa AWSプラクティスアップデート
//echo "command<>".$command."<br>\n";
		exec("$command",$LIST);
...
function update_end_html($L_NAME) {
	...
	$html  = $L_NAME['course_name']."のワード情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
	$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\">\n";
