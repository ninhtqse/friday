...
function default_html($L_NAME,$ERROR) {
	...
	//	サービス基本情報
	$sql  = "SELECT max(upd_date) AS upd_date FROM ".T_SERVICE.";";

	//	サービス・コース情報
	$sql_2  = "SELECT max(upd_date) AS upd_date FROM ".T_SERVICE_COURSE_LIST.";";
	...
	if ($local_time || $remote_time) {
		$submit_msg = "サービス情報を検証へアップしますがよろしいですか？";

		$html .= "サービス情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER[PHP_SELF]."\" method=\"POST\" name=\"del_form_all\">\n";
...
function update() {
	...
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_SERVICE, $INSERT_NAME, $INSERT_VALUE);
	}
	//	サービス・コース基本情報
	$sql  = "SELECT * FROM ".T_SERVICE_COURSE_LIST.";";
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_SERVICE_COURSE_LIST, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	//	サービス基本情報
	$sql = "TRUNCATE TABLE ".T_SERVICE.";";
	$DELETE_SQL['service'] = $sql;
	//	サービス・コース情報
	$sql = "TRUNCATE TABLE ".T_SERVICE_COURSE_LIST.";";
	...
	//	テーブル最適化
	$sql = "OPTIMIZE TABLE ".T_SERVICE.";";
	if (!$connect_db->exec_query($sql)) {
		$ERROR[] = "SQL OPTIMIZE ERROR<br>$sql";
	}
	$sql = "OPTIMIZE TABLE ".T_SERVICE_COURSE_LIST.";";
	if (!$connect_db->exec_query($sql)) {
		$ERROR[] = "SQL OPTIMIZE ERROR<br>$sql";
	}
