...
function default_html($ERROR) {
	...
	//	情報取得クエリー
	$sql  = "SELECT MAX(ms_test_type.upd_date) AS upd_date FROM ".T_MS_TEST_TYPE." ms_test_type;";
	$sql_cnt  = "SELECT DISTINCT ms_test_type.*  FROM ".T_MS_TEST_TYPE." ms_test_type;";
	...
	if ($local_time || $remote_time) {
		$submit_msg = "すらら英単語種類の情報を検証へアップしますがよろしいですか？";

		$html .= "すらら英単語種類の情報をアップする場合は、「アップする」ボタンを押してください。<br>\n";
		$html .= "<form action=\"".$_SERVER['PHP_SELF']."\" method=\"POST\">\n";
		...
		$html .= "すらら英単語種類の情報が設定されておりません。<br>\n";
	}
...
function update(&$ERROR) {
	...
	if ($result = $cdb->query($sql)) {
		make_insert_query($result, T_MS_TEST_TYPE, $INSERT_NAME, $INSERT_VALUE);
	}

	//	検証バッチDBデーター削除クエリー
	$sql  = "DELETE FROM ".T_MS_TEST_TYPE.";";
	$DELETE_SQL[] = $sql;
...
function update_end_html() {
	...
	$html  = "すらら英単語種類の情報のアップが完了致しました。<br>\n";
	$html .= "<br>\n";
	$html .= "<form action=\"".$_SERVER['PHP_SELF']."\" method=\"POST\">\n";
